class Chess {
  ...
  setOverCasing(mX, mY) { // A utiliser avec p5, dans mouseMoved
    mX -= width/2-this.config.sizeW/2;
    mY -= height/2-this.config.sizeH/2;

    if (mX>=0 && mX<this.config.sizeW
      && mY>=0 && mY<this.config.sizeH)
    {
      ...
      let column = (columnA===columnD) ? columnA+1 : null;
      ...
      let line = (lineA===lineD) ? lineA+1 : null;

      if (column && line) {
        console.log("Mouse is over case: ("+column+","+line+")");
      }
